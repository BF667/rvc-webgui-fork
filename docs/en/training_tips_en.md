...
Models always use pitch guidance in this fork.
...
Convert the wav file to embedding in advance using HuBERT. Read the wav file saved in `/logs/your-experiment-name/1_16k_wavs`, convert the wav file to 768-dimensional v2 features with HuBERT, and save in npy format in `/logs/your-experiment-name/3_feature768`.
...
- It loads v2 f0 pretrained weights from `rvc-location/pretrained_v2/`, such as `f0G48k.pth` and `f0D48k.pth`.
...
For index learning, we use the approximate neighborhood search library faiss. Read the feature value of `logs/your-experiment-name/3_feature768` and use it to learn the index, and save it as `logs/your-experiment-name/add_XXX.index`.
